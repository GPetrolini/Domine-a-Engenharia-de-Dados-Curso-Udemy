...
def carga_categories(df_categories):
    ...
    for i,df_categories_coluna in df_categories.iterrows():
        cursor.execute('''select categories_id FROM python.categories 
                       where categories_id = %s''',( df_categories_coluna['category_id'],))
...
def carga_orders(df_orders):
    ...
    # Interage no DataFrame e realiza a carga dos dados para o banco
...
df_categories = pd.read_csv(r'C:\curso-eng-dados\Python-SGBDS\Origem de Dados\Bikes\categories.csv',sep=',').fillna('')
df_products = pd.read_csv(r'C:\curso-eng-dados\Python-SGBDS\Origem de Dados\Bikes\products.csv').fillna('')
df_stores = pd.read_csv(r'C:\curso-eng-dados\Python-SGBDS\Origem de Dados\Bikes\stores.csv',sep=',').fillna('')
df_customers= pd.read_csv(r'C:\curso-eng-dados\Python-SGBDS\Origem de Dados\Bikes\customers.csv').fillna('')
#df_customers=df_customers.fillna('') # Transformação de dados (eliminando Nulos)
df_order_items = pd.read_csv(r'C:\curso-eng-dados\Python-SGBDS\Origem de Dados\Bikes\order_items.csv',sep=',').fillna('')

df_orders = pd.read_csv(r'C:\curso-eng-dados\Python-SGBDS\Origem de Dados\Bikes\orders.csv',sep=',').fillna('')
